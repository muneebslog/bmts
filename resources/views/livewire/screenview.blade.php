...
new
    ...
    public $yellowCards = [];
    public $redCards = [];
    public function mount($court_number)
    {
        $this->court_number = $court_number;

        $this->match = Game::where('court_number', $court_number)
        ...
        if (!$this->match) {
            ...
            $this->match = null;
            // dd('No active match on this court');
        }
...
{
    // Case 1: we already have a match, but it may have ended
    if ($this->match) {
        $this->match->refresh();
        ...
        // Match ended → forget it
        $this->match = null;
    }

    // Case 2: look for a new ongoing match
    $this->match = Game::where('court_number', $this->court_number)
        ->where('status', 'ongoing')
        ->first();
}
...
<div class="" wire:poll.1s="fetchMatch">
    ...
    @if ($match)
        ...
        * {
        ...
        .fullscreen-btn:active {
            ...
            <div class="scoreboard-container" id="screenview">
                <div class="footer">
                    <div class="footer-title">
                        <span>{{ $match->event->name }} {{ $match->court_number ? 'Court Number '.$match->court_number : '' }}</span>
                    </div>
                    <div class="">
                ...
                setInterval(() => {
                    fetch(window.location.href)
                        ...
                        .then(html => {
                            ...
                            const newContent = doc.querySelector('#screenview-component');
                            document.querySelector('#screenview-component').innerHTML = newContent.innerHTML;
                        });
                }, 1000);
    ...
    <style>
        * {
        ...
        .divider {
            width: 100px;
            height: 4px;
            background: linear-gradient(90deg, transparent, #0072ff, transparent);
            margin: 30px auto;
            border-radius: 2px;
        }
    ...
    <div class="container">
        ...
        <h1>No match is running in court <span class="court-number"># {{ $court_number }}</span> yet</h1>
