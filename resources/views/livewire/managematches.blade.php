   ...
   public function generateRoundsAndMatches()
{
    ...
    // ✔ If doubles: convert players → teams (ignore leftover if odd)
    if ($this->isDoubles) {
        $playersCount = intdiv($playersCount, 2); // floor division
    }

    // ✔ If no teams/players after division, block
    if ($playersCount < 1) {
    ...
    $slots = pow(2, ceil(log($playersCount, 2)));

    // ✔ Check if matches exist already
    ...
    DB::transaction(function () use ($slots) {
        ...
        for ($i = 1; $i <= $totalRounds; $i++) {
            ...
            // Only create matches for the FIRST round
            if ($i == 1) {
  ...
  @if ($match->team1 && $match->team2)
                  ...
                  <div class="flex justify-between items-center">
                    ...
                    @if ($match->status == 'ready' && $match->round->round_number)
                        <div class=" flex justify-end items-center mt-3 ">
                            <flux:button variant="danger"  wire:click="resetplayers({{$match->id}})" size='xs'  wire:confirm class=" ">Reset Players
                            </flux:button>
                        </div>
                    @endif
                    @if ($match->status == 'ready')
